...
window.addEventListener('DOMContentLoaded', () => {
  ...
  let rodadaAtual = (() => {
    const rodadasComPontuacao = resultadosFase2
      ...
      .map(r => r.rodada);
    return rodadasComPontuacao.length ? Math.max(...rodadasComPontuacao) : 26;
  })();
  
  const RODADA_MAXIMA = 31; // Define a rodada máxima
  ...
  function criarNavegacaoRodadaGrupo(grupo, rodadaParaExibir) {
    ...
    btnAnterior.addEventListener("click", () => {
      if (rodadaAtual > 26) atualizarRodada(rodadaAtual - 1);
    });
    ...
    if (rodadaAtual === 26) btnAnterior.disabled = true;
